  ...
  public function createPaymentMethod(PaymentMethodInterface $payment_method, array $payment_details) {
    ...
    if (!empty($this->getConfiguration()['cca_status']) && !empty($payment_details['cca_jwt_token'])) {
      if (empty($payment_details['cca_jwt_response_token'])) {
        throw PaymentGatewayException::createForPayment($payment_method, 'Cannot continue when CCA is enabled but not used.');
      }
      ...
      if (!$configuration->validator()
        ...
        throw PaymentGatewayException::createForPayment($payment_method, 'Response CCA JWT is not valid.');
      }
      ...
      if (isset($payload->getValue()->Payment->ExtendedData->SignatureVerification) && $payload->getValue()->Payment->ExtendedData->SignatureVerification === 'N') {
        throw PaymentGatewayException::createForPayment($payment_method, 'Unsuccessful signature verification.');
      }
    }
