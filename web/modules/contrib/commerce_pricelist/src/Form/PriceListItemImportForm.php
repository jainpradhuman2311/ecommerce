...
class PriceListItemImportForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, PriceListInterface $commerce_pricelist = NULL) {
    ...
    $sample_file_path = $this->extensionPathResolver->getPath('module', 'commerce_pricelist') . '/sample_file.csv';
    $form['#tree'] = TRUE;
    $form['csv'] = [
      '#type' => 'file',
    ...
    // Support for D11, the "file_validate_extensions" does not exist.
  ...
  public static function batchProcess($file_uri, array $mapping, array $csv_options, $price_list_id, $delete_existing, array &$context) {
    ...
    if (empty($context['sandbox'])) {
      ...
      $context['results']['skus_skipped'] = [];
      $context['results']['lines_skipped'] = [];
    }
    ...
    for ($i = 0; $i < $limit; $i++) {
      ...
      foreach (['purchasable_entity', 'price', 'currency_code'] as $required_column) {
        if (!isset($row[$required_column]) || $row[$required_column] === '') {
          $context['results']['lines_skipped'][] = $csv->key();
      ...
      try {
        ...
        if ($purchasable_entity->getEntityTypeId() === 'commerce_product_variation') {
          $context['results']['skus_skipped'][] = $purchasable_entity->getSku();
        }
        $context['results']['lines_skipped'][] = $csv->key();
        $context['results']['import_skipped']++;
        $import_count++;
  ...
  public static function finishBatch($success, array $results, array $operations) {
    ...
    if (!empty($results['error'])) {
      ...
      if (!empty($results['import_skipped'])) {
        $lines_skipped = $results['lines_skipped'];
        if (count($lines_skipped) > 40) {
          // Don't show all the skipped lines numbers as it'd break the display.
          $lines_skipped = array_slice($lines_skipped, 0, 40);
          $lines_skipped[] = '...';
        }
        $args = [
          '@lines' => implode(',', $lines_skipped),
        ];
        \Drupal::messenger()->addWarning(\Drupal::translation()->formatPlural(
          $results['import_skipped'],
          'Skipped 1 price during import (line: @lines).',
          'Skipped @count prices during import (lines: @lines).',
          $args
        ));
      }
      if (!empty($results['skus_skipped'])) {
        $args = [
          '@skus' => implode(',', $results['skus_skipped']),
        ];
        \Drupal::messenger()->addError(\Drupal::translation()->formatPlural(
          count($results['skus_skipped']),
          'Could not import the price for sku: @skus',
          'The prices for the following SKUS could not be imported: @skus',
          $args
        ));
      }
    }
