...
class CoreViewsContextualFilter extends CoreViewsFacetSourceBase {
  ...
  public function getDataDefinition($field_name) {
    ...
    if (isset($field_definition[$original_field_name])) {
      ...
      if ($field_type === 'address' && !empty($definition['field'])) {
        $address_field_component = $definition['field'];
        foreach ($property_definitions as $address_component => $address_component_definition) {
          if ($address_field_component === $original_field_name . '_' . $address_component) {
            return $address_component_definition;
          }
        }
      }
